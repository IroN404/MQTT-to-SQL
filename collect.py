...
mycursor.execute("DELETE FROM groupe5_data")
mycursor.execute("DELETE FROM groupe5_capteur")
...
def traiter_donnee(id,piece,date,ti,temp):
    ...
    sql_select_Query = "select * from groupe5_capteur"
    ...
    if test == 0:
        id_capteur += 1
        sql = "INSERT INTO groupe5_capteur (id,nom,mac,piece,emplacement) VALUES (%s, %s, %s, %s, %s)"
        val = (id_capteur,id,id,piece,"blank")
        mycursor.execute(sql, val)

    sql = "INSERT INTO groupe5_data (id,data,timestamp,capteur) VALUES (%s, %s, %s, %s)"
    sql_select_Query = "select * from groupe5_capteur"
    cursor = mydb.cursor(dictionary=True)
    cursor.execute(sql_select_Query)
    capteurs = cursor.fetchall()
